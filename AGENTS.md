...
- **Never reply on an issue.** Investigating one is in scope — reading it, tracing the code, reporting findings and a recommended fix back in the session. Posting is not: no `gh issue comment`, no closing message, no API equivalent. Issue threads are this project's public voice with its users, an agent's reply publishes under the maintainer's account, and nothing sent there can be quietly retracted. Labels, assignment and a bare close are fine when asked for. If a reply is needed, draft it for the maintainer to post.
