...
> MTG fork note: this direction arrived after the fork implemented separate
> `api_source_credential` and `service_api_credential` paths. The upstream sync
> preserves those working paths; converging them on the single `api_credential`
> model below remains required before proposing the fork implementation upstream.

## Context

At the time of this upstream decision, every authenticated request was a human
in a browser. Local password login
