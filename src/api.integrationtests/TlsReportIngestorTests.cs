...
public sealed class TlsReportIngestorTests(PostgreSqlDatabaseFixture database) : IAsyncLifetime
{
    ...
    private async Task<TlsReportIngestOutcome> IngestAsync(TlsRptParseResult parsed)
    {
        ...
        return await ingestor.IngestAsync(
            new ReportSourceContext(source.Id, source.DefaultClientId), parsed, CancellationToken.None);
    }
