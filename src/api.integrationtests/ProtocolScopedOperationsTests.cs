...
public sealed class ProtocolScopedOperationsTests(PostgreSqlDatabaseFixture database) : IAsyncLifetime
{
    ...
    private static ReportSource Source(Guid id, string name, string protocol, bool deleteAfterRetention = false)
    {
        ...
        return new ReportSource
        {
            ...
            Name = name,
            Protocol = protocol,
            Host = polled ? $"{protocol}.example.test" : null,
            Port = polled ? 993 : null,
            UseTls = polled ? (bool?)true : null,
            Username = polled ? "rua@acme.test" : null,
            PasswordEncrypted = polled ? "x" : null,
